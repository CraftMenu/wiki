...
Este guia cobre a criação de menus customizados no CraftMenu.

## Estrutura do Menu

Menus são definidos em arquivos YAML em `plugins/CraftMenu/menus/`.
...
```yaml
settings:
  cursor-sensitivity: 1.0    # Sensibilidade do mouse (1.0 = normal)
  max-yaw-offset: 61.0       # Limite horizontal em graus
  max-pitch-offset: 36.0     # Limite vertical em graus
  camera-lock-enabled: true  # Travar câmera do jogador quando menu estiver aberto
...
settings:
  visibility:
    ...
    hide_items: false        # Ocultar itens dropados
...
- Teste menus completamente antes de implantar
